...
using VectorizedRNG, Distributed; addprocs(); nprocs()
@everywhere begin;
	using Random
	using VectorizedRNG
	using RNGTest
...
@everywhere struct RN01 <: Random.AbstractRNG end
@everywhere Random.rand!(r::RN01, x::AbstractArray) = normalcdf!(randn!(local_pcg(), x))
rngnorm = RNGTest.wrap(RN01(), Float64);
@time bcj = RNGTest.bigcrushJulia(rngnorm);
minimum(minimum.(bcj))
maximum(maximum.(bcj))
