...
function initXoshiro!(
  ...
  d::UInt64,
  v::UInt64
) # P here means number of streams
  for j ∈ 1:P-1
    i = P - j
    vstoreu!(ptr, e, 8i)
    vstoreu!(ptr, z, 8 * (i + P))
    vstoreu!(ptr, d, 8 * (i + 2P))
    vstoreu!(ptr, v, 8 * (i + 3P))
...
@generated function initXoshiro(
  ::Val{P},
  ::Val{W},
  e_0::UInt64,
  z_0::UInt64,
  ...
  for p = 1:P
    ...
    for w = 1:W
      ...
      push!(
        q.args,
        :(($enew, $znew, $dnew, $vnew) = jump($eold, $zold, $dold, $vold))
      )
      eold = enew
      zold = znew
      dold = dnew
...
@inline Base.pointer(rng::MutableXoshiro) =
  Base.unsafe_convert(Ptr{UInt64}, Base.pointer_from_objref(rng))
...
@inline function getrandu64counter(rng::Xoshiro{P}) where {P}
  vloadu(
    Base.unsafe_convert(Ptr{UInt8}, pointer(rng)),
    4simd_integer_register_size() * P
  )
end
@inline function getrand64counter(rng::Xoshiro{P}) where {P}
  vloadu(
    Base.unsafe_convert(Ptr{UInt8}, pointer(rng)),
    4simd_integer_register_size() * P + 2
  )
end
@inline function getrandn64counter(rng::Xoshiro{P}) where {P}
  vloadu(
    Base.unsafe_convert(Ptr{UInt8}, pointer(rng)),
    4simd_integer_register_size() * P + 3
  )
end
...
@inline function getstate(
  ...
  GC.@preserve rng begin
    XoshiroState(
      VecUnroll((
        vloada(ptr, MM{W,8}(StaticInt{0}())),
        vloada(ptr, MM{W,8}(simd_integer_register_size()))
      )),
      VecUnroll((
        vloada(ptr, MM{W,8}(P * simd_integer_register_size())),
        vloada(ptr, MM{W,8}(simd_integer_register_size() * (1 + P)))
      )),
      VecUnroll((
        vloada(ptr, MM{W,8}(2P * simd_integer_register_size())),
        vloada(ptr, MM{W,8}(simd_integer_register_size() * (1 + 2P)))
      )),
      VecUnroll((
        vloada(ptr, MM{W,8}(3P * simd_integer_register_size())),
        vloada(ptr, MM{W,8}(simd_integer_register_size() * (1 + 3P)))
      ))
    )
  end
end
@inline function getstate(
  ...
  GC.@preserve rng begin
    ...
    XoshiroState(
      VecUnroll((
        vloada(ptr, MM{W,8}(StaticInt{0}())),
        vloada(ptr, MM{W,8}(RS)),
        vloada(ptr, MM{W,8}(RS * 2)),
        vloada(ptr, MM{W,8}(RS * 3))
      )),
      VecUnroll((
        vloada(ptr, MM{W,8}(P * RS)),
        vloada(ptr, MM{W,8}(RS * (1 + P))),
        vloada(ptr, MM{W,8}(RS * (2 + P))),
        vloada(ptr, MM{W,8}(RS * (3 + P)))
      )),
      VecUnroll((
        vloada(ptr, MM{W,8}(2P * RS)),
        vloada(ptr, MM{W,8}(RS * (1 + 2P))),
        vloada(ptr, MM{W,8}(RS * (2 + 2P))),
        vloada(ptr, MM{W,8}(RS * (3 + 2P)))
      )),
      VecUnroll((
        vloada(ptr, MM{W,8}(3P * RS)),
        vloada(ptr, MM{W,8}(RS * (1 + 3P))),
        vloada(ptr, MM{W,8}(RS * (2 + 3P))),
        vloada(ptr, MM{W,8}(RS * (3 + 3P)))
      ))
    )
  end
end
...
@inline XoshiroState(
  ...
  vier::VecUnroll{N},
  s::XoshiroState{N}
) where {N} = XoshiroState(eins, zwei, drei, vier)
@generated function XoshiroState(
  eins::VecUnroll{N},
  zwei::VecUnroll{N},
  drei::VecUnroll{N},
  vier::VecUnroll{N},
...
function randbuffer64(r::Xoshiro{P}) where {P}
  ptr = pointer(r)
  Buffer256(
    Base.unsafe_convert(Ptr{Float64}, ptr + P * 5simd_integer_register_size())
  )
end
function randnbuffer64(r::Xoshiro{P}) where {P}
  ...
  Buffer256(
    Base.unsafe_convert(
      Ptr{Float64},
      ptr + P * 5simd_integer_register_size() + 2048
    )
  )
end
...
function randubuffer64(r::Xoshiro{P}) where {P}
  ...
  Buffer256(
    Base.unsafe_convert(
      Ptr{UInt64},
      ptr + P * 5simd_integer_register_size() + 4096
    )
  )
end
